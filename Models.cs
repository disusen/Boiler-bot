...
public class BotGoal
{
    ...
    /// <summary>True if created by the user via !goal add, false if self-generated by Boiler.</summary>
...
/// <summary>
/// A belief Boiler has formed about the user through pattern recognition.
///
/// Beliefs are distinct from facts:
///   - A fact is "Gvidas lives in Vilnius" — directly stated, stable.
///   - A belief is "Gvidas tends to overcommit when excited about a project" — inferred
///     from repeated observations, held with a confidence that drifts over time.
///
/// Beliefs gate Boiler's behavior. A high-confidence belief that the user is under
/// significant stress will make Boiler avoid piling on suggestions, even if the user
/// hasn't explicitly said "back off." This is the autonomy layer.
///
/// Confidence mechanics:
///   - Each confirming observation nudges confidence up
///   - Each contradicting observation nudges it down
///   - Beliefs below RetirementThreshold for too long are marked Retired
///   - Beliefs above ActiveThreshold are behaviorally active
/// </summary>
public class BotBelief
{
    ...
    /// <summary>
    /// The belief claim in plain language.
    /// Example: "tends to self-sabotage under academic pressure"
    ///          "is more productive when habits are on streak"
    ///          "responds better to gentle nudges than direct criticism"
    /// </summary>
    public string Claim { get; set; } = string.Empty;

    /// <summary>
    /// A stable key for this belief so confirmations/contradictions can find it.
    /// snake_case, e.g. "self_sabotage_under_pressure", "productive_on_streak"
    /// </summary>
    public string BeliefKey { get; set; } = string.Empty;

    /// <summary>
    /// Current confidence. 0.0–1.0.
    /// Below 0.35 → forming, not yet behaviorally active.
    /// 0.35–0.65 → tentative, Boiler is aware but won't act strongly on it.
    /// Above 0.65 → active, influences Boiler's behavior and tone.
    /// </summary>
    public float Confidence { get; set; } = 0.4f;

    /// <summary>Domain this belief belongs to — used for prompt grouping.</summary>
    public BeliefDomain Domain { get; set; } = BeliefDomain.Behavior;

    /// <summary>
    /// What behavioral implication this belief carries when confidence is high.
    /// Written as a directive Boiler follows in its system prompt.
    /// Example: "avoid suggesting new tasks or commitments when stress signals are present"
    ///          "lean into positive reinforcement when habits are on streak — it's working"
    /// </summary>
    public string? BehavioralImplication { get; set; }

    /// <summary>How many times observations have confirmed this belief.</summary>
    public int ConfirmationCount { get; set; } = 0;

    /// <summary>How many times observations have contradicted this belief.</summary>
    public int ContradictionCount { get; set; } = 0;

    /// <summary>The observations that originally formed this belief — kept for transparency.</summary>
    public string? FormationEvidence { get; set; }

    public BeliefStatus Status { get; set; } = BeliefStatus.Forming;

    public DateTime FormedAt { get; set; } = DateTime.UtcNow;
    public DateTime LastEvidenceAt { get; set; } = DateTime.UtcNow;
    public DateTime? RetiredAt { get; set; }
}

public enum BeliefDomain
{
    Behavior = 0,       // how the user tends to act
    Emotional = 1,      // emotional patterns and triggers
    Productivity = 2,   // work/task/habit patterns
    Health = 3,         // physical/mental health patterns
    Relationships = 4,  // how the user relates to others
    Values = 5          // what the user seems to care about deeply
}

public enum BeliefStatus
{
    Forming = 0,    // confidence below active threshold — not yet behaviorally influential
